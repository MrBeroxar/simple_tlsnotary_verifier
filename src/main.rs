use std::time::Duration;
use std::fs;

use tlsn_core::{
    presentation::{Presentation, PresentationOutput},
    signing::VerifyingKey,
};

use clap::Parser;


#[derive(Parser, Debug)]
#[command(version, about)]
struct Args {
    #[clap(default_value = "data/example-json.presentation.tlsn")]
    presentation_path: String,
}

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();

    verify_presentation(&args.presentation_path).await
}

async fn verify_presentation(presentation_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    // Read the presentation from disk.
    let presentation_bytes = fs::read(presentation_path)?;
    let presentation: Presentation = bincode::deserialize(&presentation_bytes)?;

    // Placeholder for crypto provider (to be adjusted based on your setup)
    let provider = tlsn_examples::get_crypto_provider_with_server_fixture();

    let VerifyingKey { alg, data: key_data } = presentation.verifying_key();

    println!(
        "Verifying presentation with {alg} key: {}\n\n**Ask yourself, do you trust this key?**\n",
        hex::encode(key_data)
    );

    // Verify the presentation.
    let PresentationOutput {
        server_name,
        connection_info,
        transcript,
        ..
    } = presentation.verify(&provider)?;

    // The time at which the connection was started.
    let time = chrono::DateTime::UNIX_EPOCH + Duration::from_secs(connection_info.time);

    let server_name_str = match server_name {
        Some(name) => format!("{name}"),
        None => "<unknown>".to_string(),
    };

    let mut partial_transcript = transcript.unwrap();
    // Set the unauthenticated bytes so they are distinguishable.
    partial_transcript.set_unauthed(b'X');

    let sent = String::from_utf8_lossy(partial_transcript.sent_unsafe());
    let recv = String::from_utf8_lossy(partial_transcript.received_unsafe());

    println!("-------------------------------------------------------------------");
    println!(
        "Successfully verified that the data below came from a session with {server_name_str} at {time}."
    );
    println!("Note that the data which the Prover chose not to disclose are shown as X.\n");
    println!("Data sent:\n{sent}\n");
    println!("Data received:\n{recv}\n");
    println!("-------------------------------------------------------------------");

    Ok(())
}
